...
static MCGLColor glowColorForState(Startlights::State state)
{
    switch (state)
    {
    case Startlights::State::FirstRow:
        return MCGLColor(0.2f, 1.0f, 0.3f, 0.5f);   // green
    case Startlights::State::SecondRow:
        return MCGLColor(1.0f, 1.0f, 0.2f, 0.5f);   // yellow
    case Startlights::State::ThirdRow:
        return MCGLColor(1.0f, 0.3f, 0.2f, 0.5f);   // red
    case Startlights::State::Go:
        return MCGLColor(0.2f, 1.0f, 0.3f, 0.8f);   // bright green GO burst
    default:
        return MCGLColor(1.5f, 0.25f, 0.25f, 0.4f); // default red
    }
}
...
void StartlightsOverlay::render()
{
    ...
    case Startlights::State::FirstRow:
        m_alpha = 1.0;
        m_startLightGlow->setColor(glowColorForState(m_model.state()));
