...
void Scene::initializeComponents()
{
    ...
    const MCGLAmbientLight ambientLight(0.25f, 0.12f, 0.35f, 1.0f);
    const MCGLDiffuseLight diffuseLight(MCVector3dF(0.5f, -1.0f, -0.5f), 0.9f, 0.05f, 0.4f, 0.85f);
    const MCGLDiffuseLight specularLight(MCVector3dF(0.5f, -1.0f, -0.5f), 0.3f, 0.9f, 1.0f, 0.9f);
...
void Scene::createCars()
{
    ...
    for (size_t i = 0; i < carCount(); i++)
    {
        if (CarS car { CarFactory::buildCar(i, carCount(), m_game) }; car)
        {
            ...
            // GPS pin marker above human player's car (white 1 / gold 2)
            if (car->isHuman())
            {
                size_t playerIdx = (i == 0) ? 0 : (m_game.hasTwoHumanPlayers() ? 1 : 0);
                auto marker = std::make_shared<MCObject>(
                    GraphicsFactory::generatePlayerMarker(playerIdx), "PlayerMarker");
                marker->setBypassCollisions(true);
                marker->shape()->view()->setHasShadow(false);
                marker->shape()->view()->setShaderProgram(m_renderer.program("menu"));
                marker->setIsRenderable(true);
                car->addChildObject(marker, MCVector3dF(0, 0, 60));
            }
        }
    }
