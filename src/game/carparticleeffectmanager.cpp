...
void CarParticleEffectManager::doOnTrackAnimations()
{
    if ((m_car.isBraking() && m_car.speedInKmh() > ON_TRACK_ANIMATION_SPEED_MIN && m_car.speedInKmh() < 200) || m_car.isSkidding())
    {
        if (!m_car.leftSideOffTrack())
        {
            ...
            ParticleFactory::instance().doParticle(ParticleFactory::SkidSmoke, m_car.leftRearTireLocation(), m_car.physicsComponent().velocity() * 0.5f);
        }

        if (!m_car.rightSideOffTrack())
        {
            doRightSkidMark(ParticleFactory::OnTrackSkidMark);
            ParticleFactory::instance().doParticle(ParticleFactory::SkidSmoke, m_car.rightRearTireLocation(), m_car.physicsComponent().velocity() * 0.5f);
        }
    }
}
...
void CarParticleEffectManager::doOffTrackAnimations()
{
    ...
    {
        const int rate = std::max(1, 5 - static_cast<int>(speed / 50));
        if (++m_smokeCounter >= rate) // Faster emission at higher speed
        {
